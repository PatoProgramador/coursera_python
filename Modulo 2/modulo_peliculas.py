...
def encontrar_pelicula(nombre_pelicula: str, p1: dict, p2: dict, p3: dict, p4: dict,  p5: dict) -> dict:
    ...
    for i in range(0, len(pelis)) :
        if nombre_pelicula == pelis[i]["nombre"] :
            return pelis[i]
    return None
...
def encontrar_pelicula_mas_larga(p1: dict, p2: dict, p3: dict, p4: dict, p5: dict) -> dict:
    ...
    mayor = p1
    for i in range(0, len(pelis)) :
        if pelis[i]["duracion"] > mayor["duracion"] :
            mayor = pelis[i]
    return mayor
...
def encontrar_estrenos(p1: dict, p2: dict, p3: dict, p4: dict, p5: dict, anio: int) -> str:
    ...
    pelis = [p1,p2,p3,p4,p5]
    for i in range(0, len(pelis)) :
        if pelis[i]["anio"] > anio :
            if message == "" :
                message = pelis[i]["nombre"]
            else :
...
def reagendar_pelicula(peli:dict, nueva_hora: int, nuevo_dia: str, 
    ...
    for pelicula in pelis:
        if pelicula['dia'] == nuevo_dia and pelicula['hora'] == nueva_hora:
            return False

    if control_horario:
        if nueva_hora < 8 or nueva_hora > 22:
            return False
